...
async function metFactionRepRequirements(ns, faction) {
  const allAugs = await fetch(ns, `ns.Singularity.getAugmentationsFromFaction('${faction}')`,
    '/Temp/faction-augs.txt')
  const owned = await fetch(ns, 'ns.Singularity.getOwnedAugmentations(true)',
      '/Temp/getOwnedAugmentations.txt')
  const unownedAugs = allAugs.filter(a => !owned.includes(a))
  const reputation = await fetch(ns, `ns.Singularity.getFactionRep('${faction}')`,
      '/Temp/faction-rep.txt')

  const augRepReqs = await fetch(ns,
    `${JSON.stringify(unownedAugs)}.map(aug => ns.Singularity.getAugmentationRepReq(aug))`,
